...
int startGameEngine(void *ptrMsg){
    ...
    //Model* model = new Model("models/foxMC/FoxTPose.glb", translate, camera);
    Model* model = new Principal("models/foxMC/FoxTPose.glb", translate, camera);
    ...
    try {
        std::vector<Animation> animations = Animation::loadAllAnimations("models/foxMC/FoxWalk.glb", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
        std::vector<Animation> animation = Animation::loadAllAnimations("models/foxMC/FoxIdle.glb", model->GetBoneInfoMap(), model->getBonesInfo(), model->GetBoneCount());
...
//  glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	Menu menu(camera);
	LoseScreen lose(camera);
    WinScreen win(camera);
    while (isProgramRunning(ptrMsg) && !gameOver) {
        ...
        if (cambio == 1) {
            ...
            translate = glm::vec3(133.0f, OGLobj->getTerreno()->Superficie(133.0, 89.0), 89.0f);
            model->setNextTranslate(&translate);
            model->setTranslate(&translate);
            OGLobj->getLoadedText()->emplace_back(fps);
            OGLobj->getLoadedText()->emplace_back(coordenadas);
        ...
        if (cambio == 2) {
            ...
            translate = glm::vec3(140.0f, OGLobj->getTerreno()->Superficie(140.0, 90.0), 90.0f);
            model->setNextTranslate(&translate);
            model->setTranslate(&translate);
            OGLobj->getLoadedText()->emplace_back(fps);
            OGLobj->getLoadedText()->emplace_back(coordenadas);
        ...
        if (loseScreen) {
            lose.Update(menuOpcion);
            lose.Draw(); 
        }
        if (winScreen) {
            win.Update(menuOpcion);
            win.Draw();
        }
        if (menuActivo) {
            menu.Update(menuOpcion);
            menu.Draw();
        }
        if (escena != OGLobj) {
            delete OGLobj;
...
bool checkInput(GameActions *actions, Scene* scene) {
    ...
    Principal* principal = (Principal*)OGLobj;
    Dust* dust = (Dust*)OGLobj;

    /*if (dust) {
        dust->updateDustEffect(isInCave);
    }*/
    ...
    if (menuActivo) {
        ...
        if (menuOpcion == 1 && actions->action) {
			menuActivo = false;
        }
        if (menuOpcion == 2 && actions->action) {
			gameOver = true;
        }
        return true;
    }
    ...
    if (kitCollectCounter == 5) { //KIT WIN CONDITION 
		winScreen = true;
    ...
    /*if (!isInCave) {
        dust->dustys.push_back(new Billboard2D((WCHAR*)L"billboards/Dust3.png", 600, 600, 50, 50, 50, dust->cameraDetails));
    }
    if (isInCave) {
        //dust->dustys.clear();
	}*/
